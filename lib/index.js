  ...
  if (sandboxHarness) {
    ...
    async function updateConfig() {
      const vals = await Promise.all([
        ...
        envVal('DSH_DAEMON_HEALTH_INTERVAL'),
      ]);
      return {
    ...
    function watchdogScript(cfg, p, opts) {
      ...
      return '\'use strict\';\n' +
        ...
        '  const opts = fd >= 0 ? { detached: true, windowsHide: true, stdio: [\'ignore\', fd, fd], env: env } : { detached: true, windowsHide: true, stdio: \'ignore\', env: env };\n' +
        ...
        '    const child = CP.spawn(bin.cmd, args, { cwd: PROFILE_DIR, windowsHide: true, stdio: [\'ignore\', \'pipe\', \'pipe\'], shell: bin.shell, env: env });\n' +
        '    let out = \'\';\n' +
        '    let err = \'\';\n' +
        ...
        '        try {\n' +
        '          const waiter = CP.spawn(process.execPath, [__filename, \'--restart-pending\'], { detached: true, windowsHide: true, stdio: \'ignore\' });\n' +
        '          waiter.unref();\n' +
        ...
        '      const child = CP.spawn(process.execPath, [__filename], { detached: true, windowsHide: true, stdio: [\'ignore\', fd, fd] });\n' +
        '      child.unref();\n' +
        ...
        '  else { launch(); console.log(\'  launched dsh web\'); }\n' +
        '  return 0;\n' +
        '}\n' +
    ...
    function cliWrapperCmd(cfg, p) {
      return '@echo off\r\n' +
        ...
        ':http\r\n' +
        '"%NODE%" -e "fetch(\'http://127.0.0.1:%PORT%/dsh-daemon/command\',{method:\'POST\',headers:{\'content-type\':\'application/json\'},body:JSON.stringify({cmd:process.argv[1]})}).then(r=>r.json()).then(j=>{process.stdout.write(j.output||j.error||\'\');process.exit(j.ok?0:1)}).catch(e=>{process.stderr.write(String(e));process.exit(1)})" %1\r\n' +
        'exit /b %errorlevel%\r\n';
    }
