  ...
  if (sandboxHarness) {
    ...
    function watchdogScript(cfg, p, opts) {
      ...
      return '\'use strict\';\n' +
        ...
        '}\n' +
        'async function runPnpmUpdate() {\n' +
        '  const bin = pnpmBinPath();\n' +
        '  const args = [\'update\', PACKAGE_NAME, \'--registry\', NPM_REGISTRY];\n' +
        '  log(\'running \' + bin.cmd + \' \' + args.join(\' \') + \' in \' + PROFILE_DIR);\n' +
        '  return await new Promise(function (resolve) {\n' +
        '    const child = CP.spawn(bin.cmd, args, { cwd: PROFILE_DIR, stdio: [\'ignore\', \'pipe\', \'pipe\'], shell: bin.shell });\n' +
        '    let out = \'\';\n' +
        '    let err = \'\';\n' +
