  ...
  if (sandboxHarness) {
    ...
    async function doStart() {
      ...
      if (!healthy) {
        ...
        let r;
        if (win) {
          r = await sh('$proc = Start-Process -FilePath ' + psq(cfg.nodePath) + ' -ArgumentList ' + psq(cfg.dshBin) + ",'web','--port','" + String(cfg.port) + "','--no-open' -WindowStyle Hidden -RedirectStandardOutput " + psq(webLog) + ' -PassThru; $proc.Id | Out-File -Encoding ascii ' + psq(p.webPid));
        } else {
          r = await sh('nohup ' + sq(cfg.nodePath) + ' ' + sq(cfg.dshBin) + ' web --port ' + String(cfg.port) + ' --no-open >> ' + sq(webLog) + ' 2>&1 & echo $! > ' + sq(p.webPid) + '; true');
        }
