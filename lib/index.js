  ...
  if (sandboxHarness) {
    ...
    async function updateConfig() {
      ...
      return {
        ...
        healthIntervalMs: parseDuration(vals[7], 3000),
      };
    }
    ...
    function watchdogScript(cfg, p, opts) {
      ...
      const healthIntervalMs = u.healthIntervalMs || 3000;
      return '\'use strict\';\n' +
        ...
        '  const opts = fd >= 0 ? { detached: true, stdio: [\'ignore\', fd, fd], env: env } : { detached: true, stdio: \'ignore\', env: env };\n' +
        ...
        '      const child = CP.spawn(process.execPath, [__filename], { detached: true, stdio: [\'ignore\', fd, fd] });\n' +
        '      child.unref();\n' +
    ...
    async function cliWrapperPath(cfg) {
      const win = await isWindows();
      const nodeDir = cfg.nodePath.slice(0, cfg.nodePath.lastIndexOf('/'));
