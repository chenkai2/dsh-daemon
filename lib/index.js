...
// auto-start, self-healing background service - a port of `wecode daemon install`.
//
// Dual mode:
...
function apply(ctx) {
  ...
  if (sandboxHarness) {
    ...
    // ------------------------------------------------------------------
    // dsh-daemon: mirrors `wecode daemon install` for the DSH web server.
    ...
    const tools = [
      {
        ...
        description: 'Register the dsh web server as an auto-start, self-healing background service (mirrors `wecode daemon install`): writes a watchdog script and a macOS LaunchAgent (or systemd/cron on Linux) so `dsh web` starts on login, restarts after sleep/wake, and self-heals via /health checks every 30s. Starts the watchdog immediately. The currently running session is untouched.',
        parameters: {
      ...
      },
      {
        ...
        description: 'Remove the dsh daemon registration (mirrors `wecode daemon uninstall`): stop the watchdog, unload and delete the LaunchAgent plist (or systemd unit / cron entry), and remove all daemon state files. The currently running session is untouched.',
        parameters: {},
        output: { schema: { type: 'string' }, render: (args, value) => [{ type: 'text', text: String(value) }] },
      ...
      },
      {
        ...
        description: 'Recreate the dsh daemon registration after upgrading dsh or changing the port (mirrors `wecode daemon reinstall`; equivalent to uninstall then install).',
        parameters: {
          port: { type: 'number', description: 'Web server port to supervise (default: the current dsh web port, usually 3080).' },
        },
        output: { schema: { type: 'string' }, render: (args, value) => [{ type: 'text', text: String(value) }] },
      ...
      },
      {
        ...
        description: 'Show dsh daemon status (mirrors `wecode daemon status`): installed since, port, watchdog PID and liveness, manual-stop flag, web server health on the supervised port, and the last watchdog log lines.',
        parameters: {},
        output: { schema: { type: 'string' }, render: (args, value) => [{ type: 'text', text: String(value) }] },
      ...
      },
      {
        ...
        description: 'Resume daemon monitoring (mirrors daemon-aware `wecode start`): clears the stopped flag, makes sure the watchdog runs, and launches the web server if it is not healthy.',
        parameters: {},
        output: { schema: { type: 'string' }, render: (args, value) => [{ type: 'text', text: String(value) }] },
      ...
      },
      {
        ...
        description: 'Pause daemon monitoring (mirrors daemon-aware `wecode stop`): writes the stopped flag so the watchdog will not restart the server, then stops the daemon-managed web server if one is running. Never touches the currently running session.',
        parameters: {},
        output: { schema: { type: 'string' }, render: (args, value) => [{ type: 'text', text: String(value) }] },
